...
import { printerMultiSelectDropdown } from '../common/props';
import { toSubmissionArray, CustomFieldValueSubmission } from '../common/custom-fields';
...
export const startPrintAction = createAction({
    ...
    props: {
        printerIds: printerMultiSelectDropdown({ required: true }),
    ...
    async run(context) {
        ...
        const shared = toSubmissionArray(context.propsValue.sharedCustomFields ?? {});
        ...
        if (individualRaw && Array.isArray(individualRaw) && individualRaw.length > 0) {
            const individual: IndividualCustomField[] = individualRaw.map((entry) => ({
                id: Number(entry.id),
                value: toSubmissionArray(entry.value ?? {}),
            }));
