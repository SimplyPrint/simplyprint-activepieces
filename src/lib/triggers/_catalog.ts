/**
 * Full catalog of webhook-backed triggers. Mirrors every case in the backend
 * `WebhookEvent` enum (ecosystem/app/Enums/WebhookEvent.php), minus the internal
 * `test` case which is debug-only and never fires in production flows.
 *
 * Each entry becomes one trigger in the Activepieces flow builder. The factory
 * (`_factory.ts`) handles the webhook registration / delete / signature /
 * live-sample flow — per-event specs here just declare the identity of each
 * trigger.
 *
 * IMPORTANT:
 * - `event` strings must match the backend `WebhookEvent` values exactly.
 * - `name` is the internal snake_case id used by Activepieces to pin flows to
 *   this trigger. NEVER rename after publish — existing user flows will break.
...
interface TriggerSpec {
    ...
    displayName: string;
    description: string;
    /** Key into SAMPLE for a hand-crafted sample; otherwise an empty-data envelope is used. */
...
export const allTriggers = SPECS.map((spec) => {
    const data: object = spec.sampleKey
        ? (SAMPLE[spec.sampleKey] as object)
        : {};
    return createWebhookEventTrigger({
        ...
        event: spec.event,
        sampleData: envelope(spec.event, data),
    });
});
