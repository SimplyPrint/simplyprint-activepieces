...
import {
    ...
    UploadPart,
    UploadPartResponse,
} from '../src/lib/common/chunked-upload';

/**
 * Build a test double for `sendPart` that:
 *  - records every call (with the exact chunk bytes a real server would see),
 *  - replies from a caller-supplied script of responses.
 *
 * The recorded `chunk` is a COPY, because the driver passes a zero-copy
 * subarray view that could alias other chunks if inspected later.
 */
function scriptedSender(responses: UploadPartResponse[]): {
    ...
    const sendPart = async (part: UploadPart): Promise<UploadPartResponse> => {
        ...
        if (i >= responses.length) {
            throw new Error(`sendPart called ${i + 1} times but only ${responses.length} responses scripted`);
        }
        return responses[i++];
    };
...
function finalResponse(overrides: Partial<{ id: string; name: string; size: number }> = {}): UploadPartResponse {
    return {
        kind: 'final',
        file: {
...
describe('driveChunkedUpload — chunked path', () => {
    it('sends first + continue + final, threading the continueToken', async () => {
        ...
        expect(third.chunkCopy[99]).toBe(43); // 299 % 256
    ...
    it('uses zero-copy views: mutating the source post-send does not retroactively affect calls already made', async () => {
        // Verifies the driver is handing out views (expected for memory-safety
        // on multi-hundred-MB files); our scriptedSender captures a copy so we
        // can still assert the bytes-at-send-time were correct.
        const data = Buffer.alloc(200);
        data.fill(0xaa);

        let step = 0;
        const sendPart = async (part: UploadPart): Promise<UploadPartResponse> => {
            // Every byte in the view must reflect the data at the moment of the call.
...
describe('driveChunkedUpload — memory behavior', () => {
    it('handles a ~120 MiB buffer chunked at 95 MiB without copying into the driver', async () => {
        // This is the real-world "file bigger than one part" case. We don't
        // actually want a 120 MiB Buffer in the test suite's memory if we can
        // help it, so we wrap a smaller backing store with an artificial
        // length by using a typed array view over a moderate ArrayBuffer and
        // still exercise the slice/loop math. Key assertion: number of calls,
        // per-chunk sizes, and that the driver didn't materialize a copy
        // (checked via buffer identity on the underlying ArrayBuffer).
        const total = 120 * 1024 * 1024;
        const chunk = 95 * 1024 * 1024;
        const data = Buffer.alloc(total);
        // Mark first and last byte so we can assert slice boundaries without
...
async function* sourceFrom(pieces: Uint8Array[]): AsyncGenerator<Uint8Array> {
    for (const p of pieces) {
        // Defensive copy so the test assertions after yield aren't mistaken
        // for mutations on the driver's side.
...
describe('driveStreamedUpload', () => {
    ...
    it('chunks across 3 parts, threads continueToken, verifies byte-for-byte content', async () => {
        // Source delivers 50 x 5-byte pieces = 250 total; driver re-groups
        // into 100/100/50 based on declared totalSize and chunkSize.
        const input: Uint8Array[] = [];
        for (let i = 0; i < 50; i++) {
            input.push(Uint8Array.from([i * 5, i * 5 + 1, i * 5 + 2, i * 5 + 3, i * 5 + 4].map((v) => v % 256)));
        }
        const { sendPart, calls } = scriptedSender([
            { kind: 'continue', continueToken: 'tok-1' },
            { kind: 'continue', continueToken: 'tok-2' },
    ...
    it('throws if Content-Length undercounted (stream keeps yielding after final)', async () => {
        // Trickier variant: source yields more pieces AFTER the driver has
        // already collected enough bytes to satisfy totalSize, so the leftover
        // arrives via the post-final drain loop rather than pending.
        const input = [
        ...
        ).rejects.toThrow(/Content-Length mismatch.*source still held 50 buffered bytes|kept delivering more/);
    });
    ...
    it('coalesces many small pieces into chunkSize parts', async () => {
        // 100 x 3-byte pieces = 300 bytes, chunk at 100 → 3 parts of 100.
        const input: Uint8Array[] = [];
        for (let i = 0; i < 100; i++) {
    ...
    it('handles pieces spanning chunk boundaries without dropping bytes', async () => {
        // Source yields one 170-byte + one 30-byte piece = 200 bytes; chunk 100.
        const input = [
...
describe('extractFilesApiId', () => {
    it('parses the canonical {file: {id, name, size, expires_at}} shape', () => {
        ...
        expect(parsed).toEqual({ id: 'abc', name: 'x.gcode', size: 42, expiresAt: '2026-01-01T00:00:00Z' });
    });
